...
export function CurrencyStatus({ className = '', showDetails = false }: CurrencyStatusProps) {
  const { exchangeRate, lastUpdated, isLoading, error, refreshRate } = useCurrencyContext();
      ...
      <CardContent className="p-4">
        ...
        <div className="space-y-3">
          <div className="text-center">
            <div className="text-sm text-muted-foreground mb-1">1 USD equals</div>
            <div className="text-3xl font-bold text-green-600">₦{exchangeRate.toFixed(2)}</div>
          </div>
          ...
          <div className="text-xs text-muted-foreground pt-2 border-t text-center">
            ⚡ Rates update automatically every hour<br/>
