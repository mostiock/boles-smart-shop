...
export function SearchComponent({ onSearchResults, className }: SearchComponentProps) {
    ...
    <div className={`relative ${className}`}>
      ...
      {showSuggestions && (
        ...
        >
              ...
              {recentSearches.slice(0, 3).map((search, index) => {
                ...
                return (
                  <button
                    key={`recent-${search}`}
                    className={`w-full text-left px-3 py-2 hover:bg-gray-50 rounded-md transition-colors ${
                      selectedIndex === adjustedIndex ? 'bg-[#43abc3] text-white' : ''
                    }`}
          ...
          {/* Popular Categories */}
          <div className="p-2 border-t border-gray-100">
            <div className="text-xs font-medium text-gray-500 mb-2">Popular Categories</div>
            <div className="flex flex-wrap gap-1">
              {['Smart Lighting', 'Security Cameras', 'Smart Speakers', 'Smart Locks'].map((category) => (
                <Badge
                  key={category}
                  variant="outline"
